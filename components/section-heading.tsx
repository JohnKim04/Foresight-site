...
export function SectionHeading({ id, title, eyebrow, description }: SectionHeadingProps) {
  return (
    <div className="max-w-2xl">
      {eyebrow ? <p className="text-sm font-medium text-indigo">{eyebrow}</p> : null}
      <h2 id={id} className="mt-2 font-display text-4xl tracking-tight text-ink">{title}</h2>
      {description ? (
